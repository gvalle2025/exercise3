...
garden_harvest %>%
  ...
  summarize(wt_total = sum(wt_lbs)) %>%
  pivot_wider(names_from = day_of_week, values_from = wt_total)
...
garden_harvest %>% 
  ...
  mutate(wt_lbs = wt_total * .00220462) %>% 
...
```
  ...
  You can find out how much money you saved for each vegetable type by 
...
garden_harvest %>% 
  ...
  group_by(variety) %>% 
  summarize(wt_total = sum(weight) * .00220462, first_harvest_date = min(date)) %>%
  ggplot(aes(x = wt_total, y = fct_rev(fct_reorder(variety, first_harvest_date, median))))+
           geom_col()
...
garden_harvest %>% 
  mutate(lowercase = str_to_lower(variety),
         length_of_name = str_length(variety)) %>% 
  arrange(vegetable, variety, length_of_name) %>% 
...
```{r}
Trips %>% 
  mutate(event_at_time = hour(sdate) + ((minute(sdate)/60))) %>% 
  ggplot(aes(x=event_at_time))+
...
Stations %>% 
  left_join(Trips,
         by=c("name"="sstation")) %>% 
  group_by(name) %>% 
  summarise(total_departures = n(), lat, long) %>% 
ggplot(aes(x = long,y = lat, color = total_departures)) +
  geom_point(aes(size = total_departures))
```
